...
function onUse(cid, item, fromPosition, itemEx, toPosition)
        ...
        if chance >= math.random(0,100) or item.actionid >= 1000 or (item.actionid == 500 and math.random(0,100) <= 25) then
           if level+1 > gain.maxlvl then
              ...
              doPlayerSendTextMessage(cid, 22,"Your item has been upgrated to [+"..(level+1).."].")
           ...
           elseif level > 0 then
               if level == 1 then
                   ...
                   doPlayerSendTextMessage(cid, 22,"Your item back to normal.")
               else
                   setItemName(itemEx.uid, getItemNameById(itemEx.itemid)..' +'..(level-1))
                   doPlayerSendTextMessage(cid, 22,"Your item back to [+"..(level-1).."].")
               end
              if isArmor(itemEx) == TRUE then
